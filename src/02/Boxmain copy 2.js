const Boxmain = () => {
    ...
    for(let item of mvlist) {
        let temptag ;
        let inten = '⏺️' ;
        if (item.rankInten > 0) inten = '⬆️' ;
        else if (item.rankInten < 0) inten = '⬇️';

        temptag = <div className="rowDiv" key={item.movieCd}>
            <span className="col" id="colrank">{item.rank}</span>
            <span className="col" id="colmovieNm">{item.movieNm}</span>
            <span className="col" id="colsalesAmt">{parseInt(item.salesAmt).toLocaleString('ko-KR')}</span>
            <span className="col" id="colrankInten">{inten}</span>
        </div> ;
        divTags.push(temptag);
    }
